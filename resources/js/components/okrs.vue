            ...
            <div class="col-md-6 okr-block" v-for="okr in okrs">
               ...
               <div class="card border-okr-card border" :class="okr.status == 'Completed'? 'border-success':''" >
                    ...
                    <div class="card-body">
                        ...
                        <div><strong :class="okr.status">{{ ' Status: '+ okr.status }}</strong> <br> <span v-html="okr.description"></span> </div>
                        <div class="row" style="margin-top:20px; white-space: pre-line;">
                            ...
                            <div class="col-10">
                                <b-progress class="mt-2" :max="getStatusCountForKrs(okr).total" show-value>
                                    <b-progress-bar :value="getStatusCountForKrs(okr).proposed" variant="secondary" title="completed"></b-progress-bar>
                                    <b-progress-bar :value="getStatusCountForKrs(okr).active" variant="primary"></b-progress-bar>
                                    <b-progress-bar :value="getStatusCountForKrs(okr).hold" variant="danger"></b-progress-bar>
                                    <b-progress-bar :value="getStatusCountForKrs(okr).completed" variant="success"></b-progress-bar>
                                </b-progress>
                            </div>
                            <div class="col-2">
  ...
  export default {
      ...
      methods:{
          ...
          getStatusCountForKrs(okr){
              let total = okr.krs.length
              let completed = okr.krs.filter(function(item ){ return item.status =="Completed"}).length;
              let hold = okr.krs.filter(function(item ){ return item.status =="Hold"}).length;
              let proposed = okr.krs.filter(function(item ){ return item.status =="Proposed"}).length;
              let active = okr.krs.filter(function(item ){ return item.status =="Active"}).length;
